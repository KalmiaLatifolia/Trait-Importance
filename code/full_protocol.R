...
# Table of Contents
################################################################################
# 1) Load and format Bioacoustic data
# 2)
# 3)
# 4)
...
x <- terra::extract(Nitrogen$s6v3_mean, sites.buffer, method="simple", exact=TRUE, na.rm=TRUE, fun=mean)
x <- cbind(sites.buffer, x[c("s6v3_mean")])
colnames(x)[colnames(x)=="s6v3_mean"] <- "Nitrogen"
x$Nitrogen[x$Nitrogen == 0] <- NA
foliarTraits <- x # 649 x 5
...
x <- terra::extract(LMA$s6v3_mean, sites.buffer, method="simple", exact=TRUE, na.rm=TRUE, fun=mean)
x <- cbind(sites.buffer, x[c("s6v3_mean")])
colnames(x)[colnames(x)=="s6v3_mean"] <- "LMA"
x$LMA[x$LMA == 0] <- NA
x <- as.data.frame(x)
foliarTraits <- merge(foliarTraits, x) # 649 x 6
...
x <- terra::extract(Cellulose$s6v3_mean, sites.buffer, method="simple", exact=TRUE, na.rm=TRUE, fun=mean)
x <- cbind(sites.buffer, x[c("s6v3_mean")])
colnames(x)[colnames(x)=="s6v3_mean"] <- "Cellulose"
x$Cellulose[x$Cellulose == 0] <- NA
x <- as.data.frame(x)
foliarTraits <- merge(foliarTraits, x) # 649 x 7
